...
#準備獨變相
cols = whole_data.columns
# data_the_day = whole_data.iloc[:,cols.get_loc("HOST_LAMIGO桃猿"):cols.get_loc("降水量(mm)")+1]
...
analysis_data = pd.concat([whole_data.iloc[:,0:cols.get_loc("降水量(mm)")],previous_data],axis=1).iloc[shift_days:,:].reset_index(drop=True)
analysis_data.drop(["Previous_index.1","STADIUM"],axis=1,inplace=True)
print(analysis_data.columns)
analysis_data.to_excel("分析資料.xlsx")
